...
from config import INPUT_DIR, OUTPUT_DIR,GPX_DIR, RES_CSV_DIR, CSV_DIR, CACHE_LOC_DIR, RES_CSV_GH_OP_DIR, BATCH_OUTPUT_DIR

from annomize import anonymize_column_values

def write2file( data, filename='log.txt', mode='a'):
	full_op_path = pathlib.Path(OUTPUT_DIR, filename)
	f = open(full_op_path, mode)
	f.write(data+'\n')
	f.close()
